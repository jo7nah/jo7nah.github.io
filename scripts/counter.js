...
function onSuccess(response) {
    if(JSON.parse(response)['value'] < 1000) {
        document.getElementById('tap-button-count').innerHTML = JSON.parse(response)['value'];
    } else if (1000 <= JSON.parse(response)['value'] < 1000000) {
        document.getElementById('tap-button-count').innerHTML = Math.floor(JSON.parse(response)['value'] / 1000) + '.' + Math.floor((JSON.parse(response)['value'] % 1000) / 100) + "K";
    }
}
